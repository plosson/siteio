...
- **`/mcp`** — the full web-file editing tools (`site_info`, `list_files`, `read_file`, `write_file`, `write_url`, `delete_file`, `deploy_site`) over a per-grant `StagingStore`. Pure MCP functions: the AI edits through tool calls. `write_url(path, url)` has the agent fetch a URL server-side into a staged web file (the clean way to add images/binary assets without inlining) — SSRF-guarded (`assertSafePublicUrl`: http(s) only, no private/reserved addresses, no redirects, size-capped).
